...
required_packages <- c("shiny", "ggplot2", "scales", "tidyverse", "googlesheets4")
...
# ===============================================
cat("🔐 Налаштування автентифікації Google Sheets...\n")

# Перевірка наявності service account ключа
service_account_file <- "service-account.json"

if (file.exists(service_account_file)) {
  # Використовуємо service account (для headless середовищ)
  cat("🔑 Використання Service Account з файлу:", service_account_file, "\n")
  googlesheets4::gs4_auth(path = service_account_file)
  cat("✅ Авторизовано через Service Account\n\n")
} else if (dir.exists(".secrets") && length(list.files(".secrets", pattern = "*.rds")) > 0) {
  # Використовуємо збережені OAuth токени
  cat("🔑 Використання збережених OAuth токенів\n")
  tryCatch({
    googlesheets4::gs4_auth(cache = ".secrets", email = TRUE)
    cat("✅ Авторизовано через OAuth\n\n")
  }, error = function(e) {
    cat("⚠️  Помилка OAuth, перехід на публічний режим\n")
    googlesheets4::gs4_deauth()
  })
} else {
  # Non-authenticated режим (для публічних таблиць)
  cat("ℹ️  Публічний режим (non-authenticated)\n")
  cat("   ✓ Працює для публічних таблиць\n")
  cat("   ✗ Для приватних таблиць потрібно:\n")
  cat("     • Service Account (service-account.json)\n")
  cat("     • АБО зробити таблицю публічною\n\n")
  googlesheets4::gs4_deauth()
  cat("✅ Готово до читання публічних таблиць\n\n")
}
```
