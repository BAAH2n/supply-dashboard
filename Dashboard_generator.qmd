...
# Спроба використати збережені токени з .secrets
# Якщо токенів немає - використовуємо deauth (для публічних таблиць)
if (dir.exists(".secrets") && length(list.files(".secrets", pattern = "*.rds")) > 0) {
  # Використовуємо збережені токени
  googlesheets4::gs4_auth(cache = ".secrets", email = TRUE)
  cat("✅ Використано збережені токени автентифікації\n\n")
} else {
  # Якщо токенів немає - попередження та deauth
  cat("⚠️  УВАГА: Токени автентифікації не знайдено!\n")
  cat("   Буде спроба доступу як non-authenticated user.\n")
  cat("   Це працює лише для ПУБЛІЧНИХ таблиць.\n\n")
  cat("   Якщо таблиця ПРИВАТНА, виконайте перед рендерингом:\n")
  cat("   > Rscript -e 'googlesheets4::gs4_auth(email=TRUE, cache=\".secrets\")'\n\n")
