# ===============================================
# SUPPLY DASHBOARD - CONFIGURABLE VERSION
# ===============================================
# 1. Load your data: long_data <- read.csv('your_file.csv')
  ...
  div(class = paste("kpi-box-small", glow_class),
      ...
      if(show_target && !is.na(change_data$target)) {
        div(style = "font-size: 11px; color: #6c757d;",
            paste0("Target: ", if(!is_higher_good) "≤" else "", change_data$target, 
                   if(is_percent) "%" else ""))
      },
      div(class = "kpi-change", 
        ...
        if(nda$total > 0) {
          div(
            div(style = "font-size: 18px; margin-bottom: 8px;",
                paste0("⚠️ ALERT: ", nda$total, " Violations detected")),
            div(style = "font-size: 14px; opacity: 0.9;",
                ...
                if(nda$change != 0) {
                  span(style = paste0("margin-left: 15px; color: ", 
                       ...
                       paste0(ifelse(nda$change > 0, "↑", "↓"), " ", abs(nda$change), " vs previous"))
                }
            )
          )
        } else {
...
generate_dashboard_image <- function(output_dir = "images", filename = NULL, delay = 3) {
  ...
  cat("✓ Dashboard image saved to:", filepath, "\n")
  return(filepath)
}
...
cat("\n")
cat("==============================================\n")
cat("🚀 SUPPLY DASHBOARD\n")
cat("==============================================\n")
cat("Chart history period:", CHART_HISTORY_MONTHS, "months\n")

# Generate static image
cat("\n📸 Generating dashboard screenshot...\n")
image_path <- generate_dashboard_image()

cat("\n🌐 Opening interactive dashboard in RStudio Viewer...\n")
